<template>
  <el-container class="table-all" v-loading="load" v-if="cfg && cfg.cols">
    <el-main class="table-main">
      <com-btns v-if="(cfg.tableFormBtns && cfg.edit) || cfg.showBtns" @outdata2="outdata2" :btns="cfg.tableFormBtns"></com-btns>
      <div v-if="cfg.edit" style="margin-bottom:10px;color:#FF0000;font-size: 12px;">
        以下信息均为为必填项
      </div>
      <el-table
        class="con-el-table"
      ...
      <div v-if="cfg.addOneCol && cfg.edit" :style="{ width: cfg.addWidth + 'px' }" class="add_table_col" @click="addOneCol">+添加一条</div>
    </el-main>
...
import uploadForCmposite from "../uploadForCmposite";
...
export default {
  ...
  methods: {
    outdata2(v) {
      $c.log(v, "eeee87987987987");
      if (v.FILE_URL && v.FILE_NAME) {
        this.cfg.tableFormBtns.intem.dataFormRun({ FILE_URL: v.FILE_URL, FILE_NAME: v.FILE_NAME }).then(res => {
          $c.log(res);
          this.val = res;
        });
      }
